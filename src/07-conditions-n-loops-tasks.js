...
function isInsideCircle(circle, point) {
  ...
  && Math.sqrt((point.y - circle.center.y) ** 2 + (point.x - circle.center.x) ** 2) < circle.radius
  );
}
...
function findFirstSingleChar(str) {
  ...
  const noRepeat = Object.keys(o).filter((e) => o[e] === 1);
  return noRepeat[0] || null;
}
...
function isCreditCardNumber(ccn) {
  const arr = ccn.toString()
    ...
    .map((e) => (e >= 10 ? e.toString().split('').map(Number).reduce((a, c) => a + c) : e))
    .reduce((a, c) => a + c);
  return !(arr % 10);
}
...
function getDigitalRoot(num) {
  if (num <= 9) return num;
  const n = num.toString().split('').map(Number).reduce((a, c) => a + c);
...
function getMatrixProduct(m1, m2) {
  const [r1, r2, c1, c2, res] = [m1.length, m2.length, m1[0].length, m2[0].length, []];
  if (c1 !== r2) throw new Error('matrices are not compatible');
  for (let i = 0; i < r1; i += 1) res[i] = [];
  for (let k = 0; k < c2; k += 1) {
    for (let i = 0; i < r1; i += 1) {
      let t = 0;
      for (let j = 0; j < r2; j += 1) t += m1[i][j] * m2[j][k];
      res[i][k] = t;
    }
  }
  return res;
